...
export function VehicleForm({ 
  ...
  const [formData, setFormData] = React.useState<VehicleFormData>({
    ...
    electricRange: initialData?.electricRange,
    standardEquipment: initialData?.standardEquipment || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate leasing rates
    const hasValidRates = Object.values(formData.leasingRates).some(rate => rate > 0);
    if (!hasValidRates) {
      toast.error('Bitte geben Sie mindestens eine gültige Leasingrate ein');
      return;
    }

    // Include electric range in the submission if it exists
    const vehicleData = {
      ...formData,
      electricRange: formData.fuelType === 'elektro' ? formData.electricRange : undefined,
    };
